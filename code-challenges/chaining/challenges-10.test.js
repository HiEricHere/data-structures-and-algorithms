...
const count = (target, input) => {
  ...
  input.forEach( element => {
    if ( element.includes( target ) ){
      totalCount += element.reduce( ( counter, value ) => {
        if ( value === target ){
          return ++counter;
        } else {
          return counter;
...
const totalSum = (input) => {
  let newArray = input.reduce( ( arrayPiece, element ) => {
    return arrayPiece.concat(element);
  }, []);
  return newArray.reduce( ( counter, element ) => {
  ...
  input.forEach( array => {
    if ( array.length ){
      let filteredArr = array.filter( value => {
        return ( !(value % 5) && (typeof(value) === 'number') );
      });
      let squaredArr = filteredArr.map( value => {
        return Math.pow( 2,value );
      });
      answer.push(squaredArr);
    } else {
      answer.push([]);
    }
  });
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach( character => {
    if ( character.gender !== 'n/a' && ( character.gender === 'male' || character.gender === 'female' ) ) array.push( character.name );
  });
